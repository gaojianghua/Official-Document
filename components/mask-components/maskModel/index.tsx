    ...
    return (
        <div className={clsx(styles.model, 'dflex', 'jcenter', 'flexcolumn')}>
            <div className={clsx(styles.title, 'dflex', 'jcenter', 'acenter', 'positionrelative')}>
                ...
                <CloseCircleOutlined className={clsx(styles.close, 'cur', 'positionabsolute')} onClick={store.model.modelData.cancel} />
            </div>
            <div className={clsx('p3')}>
                {store.model.modelData.children}
            </div>
            <div className={clsx('dflex', 'jsa', 'acenter', 'mtauto', 'pb2')}>
                <Button className={clsx(styles.cancelBtn)} type='primary' onClick={store.model.modelData.cancel}>
                    取 消
                </Button>
                <Button className={clsx(styles.confirmBtn)} type='primary' onClick={store.model.modelData.confirm}>
                    确 认
                </Button>
            </div>
        </div>
